...
import { 
  getComentariosTreinamento, 
  criarComentario, 
  editarComentario, 
  deletarComentario 
} from '../services/comentariosService';

const ComentariosSection = ({ treinamentoId, onComentarioChange }) => {
  const { user } = useAuth();
  ...
  const carregarComentarios = async () => {
    setLoading(true);
    try {
      const result = await getComentariosTreinamento(treinamentoId);
      if (result.error) {
  ...
  const handleEnviarComentario = async (e) => {
    ...
    setEnviando(true);
    try {
      const result = await criarComentario(treinamentoId, novoComentario, user.id);
      if (result.error) {
        setError(result.error);
      } else {
  ...
  const handleEditarComentario = async (comentarioId) => {
    ...
    try {
      const result = await editarComentario(comentarioId, comentarioEditado, user.id);
      if (result.error) {
        setError(result.error);
      } else {
        setComentarios(prev => 
  ...
  const handleDeletarComentario = async (comentarioId) => {
    ...
    try {
      const result = await deletarComentario(comentarioId, user.id);
      if (result.error) {
        setError(result.error);
      } else {
