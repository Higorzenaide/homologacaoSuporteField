...
const CurtidasButtonNoticia = ({ noticiaId, onCurtidaChange }) => {
  ...
  const carregarDados = async () => {
    ...
    try {
      const [curtidaResult, contadorResult, usuariosResult] = await Promise.all([
        user ? verificarCurtidaNoticia(noticiaId, user.id) : { curtido: false },
        contarCurtidasNoticia(noticiaId),
        getUsuariosCurtiramNoticia(noticiaId, 5)
      ]);
  ...
  const handleToggleCurtida = async () => {
    ...
    setProcessando(true);
    try {
      const result = await toggleCurtidaNoticia(noticiaId, user.id);
      if (result.error) {
        ...
        const usuariosResult = await getUsuariosCurtiramNoticia(noticiaId, 5);
        if (!usuariosResult.error) {
          setUsuariosCurtiram(usuariosResult.data);
        }
      }
...
export default CurtidasButtonNoticia;
